...
import com.example.holidaytest4.activities.MapActivity;
import com.example.holidaytest4.activities.SearchActivity;
import com.example.holidaytest4.adapter.ItemClickListener;
import com.example.holidaytest4.adapter.MyPagerAdapter;
import com.example.holidaytest4.adapter.SceneryAdapter;
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.weather_home_tv:
                Toast.makeText(getContext(),"返回手机桌面，即可查看！",Toast.LENGTH_SHORT).show();
                break;
            case R.id.wc_home_tv:
                Toast.makeText(getContext(),"问问人......",Toast.LENGTH_SHORT).show();
                break;
            case R.id.translation_home_tv:
                Toast.makeText(getContext(),"下载有道词典！",Toast.LENGTH_SHORT).show();
                break;
        ...
        switch (list.get(position).getName()){
            case "光明顶":
                if(getActivity()!=null) {
                    ...
                    intent.putExtra("Latitude",30.132266);
                    intent.putExtra("Longitude",118.169168);
                    startActivity(intent);
                }
                break;
            case "飞来石":
                if(getActivity()!=null) {
                    Intent intent = new Intent(getActivity(), MapActivity.class);
                    intent.putExtra("Latitude",30.135566);
                    intent.putExtra("Longitude",118.163966);
                    startActivity(intent);
                }
                break;
            case "一线天":
                if(getActivity()!=null) {
                    Intent intent = new Intent(getActivity(), MapActivity.class);
                    intent.putExtra("Latitude",30.126444);
                    intent.putExtra("Longitude",118.169330);
                    startActivity(intent);
                }
                break;
            case "迎客松":
                if(getActivity()!=null) {
                    Intent intent = new Intent(getActivity(), MapActivity.class);
                    intent.putExtra("Latitude",30.122344);
                    intent.putExtra("Longitude",118.175755);
                    startActivity(intent);
                }
                break;
            case "步仙桥":
                if (getActivity()!=null) {
                    Intent intent = new Intent(getActivity(),MapActivity.class);
                    intent.putExtra("Latitude",30.131802);
                    intent.putExtra("Longitude",118.155638);
                    startActivity(intent);
                }
        }
    }
